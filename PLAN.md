...
- 里程碑：S1 假镜像链路 → S3-00 真内核在板子串口打出第一行字并**故意看到 panic**（init_IRQ "No interrupt controller found."）→ S3-01 earlycon 出字 + 故意 panic 验收 → S3-02 定时器链（jiffies 动）→ S3-03 Xh3irq 外设中断 → S3-04 真 console → 进 shell。
