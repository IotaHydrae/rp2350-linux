...
| 脚本 | 作用 | 用法 |
|---|---|---|
| `pc-locate.sh` | 把 PC 值翻译成符号/源文件/反汇编片段 | `scripts/pc-locate.sh <vmlinux> <pc> [基址]` |
...
| `log-analyze.sh` | 串口日志体检：提取告警/panic 关键词、Call Trace 地址批量翻译（带行号） | `scripts/log-analyze.sh <vmlinux> <日志> [基址]` |
