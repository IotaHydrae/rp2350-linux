/*
 * S4 工程 6 (05_buildroot-rootfs) bootloader（分区表版 · 最小化变体）：
 *   分区 0 = KERNEL（3MB，复用 S4-04 kernel-Image）-> PSRAM 0x11000000
 *   分区 1 = DTB（64K，dts 编译产物）   -> PSRAM 0x11700000（顶部）
 *   分区 2 = ROOTFS（1MB，raw ext2 镜像，不再包 cpio）-> PSRAM 0x11300000
...
#define ROOTFS_LOAD_ADDR 0x11300000u
...
#define CPU_SPEED_MHZ 225
    ...
    if (km || dm || im) {
        printf("verify FAILED: kernel=%u dtb=%u rootfs=%u - halting\n",
