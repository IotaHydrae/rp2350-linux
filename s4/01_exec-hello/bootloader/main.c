/*
 * S4 工程 2 (01_exec-hello) bootloader（分区表版 · 最小化变体）：
 *   分区 0 = KERNEL（3MB，本工程 kernel-Image）-> PSRAM 0x11000000
 *   分区 1 = DTB（64K，dts 编译产物）   -> PSRAM 0x11700000（顶部）
 *   分区 2 = ROOTFS（1MB，rootfs.cpio） -> PSRAM 0x11300000
 *             DTB chosen 的 linux,initrd-start/end 写死这个地址（固定约定）
...
#define INITRAMFS_LOAD_ADDR 0x11300000u
...
int main(void) {
    ...
    uint32_t initramfs_flash, initramfs_size;
    if (!find_partition(0, &kernel_flash, &kernel_size) ||
        !find_partition(1, &dtb_flash, &dtb_size) ||
        !find_partition(2, &initramfs_flash, &initramfs_size)) {
    ...
    copy_image("initramfs", initramfs_flash, initramfs_size, INITRAMFS_LOAD_ADDR);
    ...
    uint32_t im = verify_copy("initramfs", initramfs_flash, initramfs_size, INITRAMFS_LOAD_ADDR);
    printf("verify: kernel mismatches=%u, dtb mismatches=%u, initramfs mismatches=%u\n",
