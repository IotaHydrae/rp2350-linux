...
**容易混淆的一点（用户 2026-08-28 问过）**：init_IRQ panic 消失**不是**定时器驱动的功劳，是两件独立的事。机制上：`handle_arch_irq` 是全局异常分发入口，由中断控制器驱动设置；`riscv,cpu-intc` 节点让内核自带的 `irq-riscv-intc.c` 被加载，它的 `riscv_intc_init()` 注册 irq domain 并 `set_handle_irq()`——`handle_arch_irq` 非空，panic 就不触发。这一步跟定时器驱动没关系。时间上：panic 在 `init_IRQ()`（启动早期）检查，定时器驱动在 `time_init()`（更晚）才注册 clocksource/clockevent，两者差着阶段。正确因果链：**cpu-intc 节点 → handle_arch_irq 就位 → 过 init_IRQ panic → 定时器驱动借 intc domain 拿 MTIP 中断号 → jiffies 动**。定时器驱动是 cpu-intc 的"受益者"不是"原因"。
...
### 补：clockevent 的"闹钟循环"（rp2350_timer_interrupt 在做什么）
...
static irqreturn_t rp2350_timer_interrupt(int irq, void *dev_id)
{
	...
	csr_clear(CSR_IE, IE_TIE);      /* 1. 关掉定时器中断使能位 */
	evdev->event_handler(evdev);    /* 2. 喊内核一声："到点了" */

	return IRQ_HANDLED;
}
```

这是 clockevent 的"闹钟到点"处理函数，两行各管一件事：

- **先关 `IE_TIE`（防电平风暴）**：MTIP 是电平型中断，只要 `MTIME >= MTIMECMP` 就一直 pending。只处理不关的话，中断返回瞬间 CPU 会立刻再进中断。`IE_TIE`（MIE 的定时器使能位）是"允许定时器中断进来"的开关，先关掉，MTIP 即使 pending 也不会触发；等下一次 `set_next_event` 写新 MTIMECMP 时再开（关-开配对，中间隔着"设好下一次到期时间"）。
- **`event_handler` 是内核注册的回调**（tick 层的 `tick_handle_periodic` / `hrtimer_interrupt`）：更新 jiffies、跑到期定时器、处理软中断。驱动不关心内核具体做什么，只负责"到点了喊一声"——这是 clockevent 的角色边界。

完整循环：`set_next_event`（写 MTIMECMP：先全 1 → 高半 → 低半 + 开 TIE）→ 时间到 MTIP 置位进中断 → `rp2350_timer_interrupt`（关 TIE → event_handler）→ 内核设下一个到期时间 → 回到 `set_next_event`。类比：`set_next_event` 是定闹钟，`rp2350_timer_interrupt` 是闹钟响了——先按掉铃声（关 TIE），再起床干活（event_handler），然后定下一个。
