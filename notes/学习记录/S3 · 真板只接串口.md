...
flowchart LR
    ...
    K -.->|无 timer → jiffies 冻结| H["预期挂点<br/>记录最后一行日志"]
```

图例：bootloader 骨架 / PSRAM 初始化 / 跳转协议来自 S1（已有）；实线边是 S3-00 新增：真内核加载、DTB 传递、最小 DTB、earlycon。承重部件：真内核加载、最小 DTB、earlycon 出字。
