...
## 联跑现象（2026-08-21 · S1 通关）

bootloader（USB）：`PSRAM available: 1, size: 8388608` → `copy 4096 bytes: flash 0x10010000 -> PSRAM 0x11000000` → `copy done. first bytes: 37 f1 7f 11`（= `lui sp`，假镜像第一条指令）→ `jump to 0x11000000 (a0=0, a1=NULL)`。

假镜像（UART）：`[Fake Image] running from PSRAM!`、`a0 (hartid) = 0x00000000`、`a1 (dtb) = 0x00000000`、`self-check: word at 0x11000000 = 0x117ff137`（与拷贝内容一致）→ looping。

三个方向同时验证：memcpy 拷贝、指令从 PSRAM 取指、数据从 PSRAM 读回。
