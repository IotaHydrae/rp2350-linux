...
**本阶段拍过的决策**：
- 镜像存放：先固定偏移 64KB（已验证闭环，归档 `s1/fixed-offset/`）→ 后升级为 picobin 分区表（partition 0 @ 64K，主线在 `s1/partition-table/`）→ 对齐 picotool 分区烧录、为多分区铺路
...
- 烧完带内嵌分区表的固件后，设备要**重启再进 BOOTSEL**，`picotool -p` 才能读到分区表（第一次会报 `There is no partition table on the device`——PT 其实已烧进去，是设备侧缓存没刷新）。
- 反汇编别手算机器码——以 objdump + gdb + 汇编器三者一致为准（完整方法见 `RISC-V调试与反汇编速查.md`）。

## 假镜像闭环：S1 验收

假镜像 373 字节，入口 `_start` 在 `0x11000000`：摆栈 → `a0/a1` 原封不动就是 C 函数参数 → 打印 + 自检 → 死循环。bootloader（分区表版）从 picobin 分区表定位 FAKE 分区（flash 64K 偏移）拷 4KB 到 PSRAM，清 MIE，按协议跳转。
...
随后**分区表版**（`s1/partition-table/`）联跑同样 PASS：bootloader 打印 `partition sectors [16, 31] -> flash 0x10010000`，其余现象一致（日志见 `实验日志/2026-08-21_S1分区表版联跑.md`）。
...
- Q4：换一块 PSRAM 在 GPIO19、镜像放 flash 1MB 的板子，要动哪几处？（分区表版）
  <details><summary>参考答案</summary>板卡头 CS 改 19、`partition_table.json` 的 start 改 "1M"；bootloader 不用改（它读分区表），烧录还是 `-p 0`，fake-image 不用动（链接地址没变）。这就是分区表的价值：改位置只动一处。</details>
...
> 原始日志存档：`../实验日志/`（本阶段 4 份，含 B 板异常、旧工程对照、新板 PASS、S1 联跑）。
